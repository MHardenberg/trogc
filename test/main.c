...
int main() {
        ...
        f_plotv("Test vector", &vx, &vy0, "x", labels[0]);
        f_plotvs("Test vectors", &vx, vs, 2, "x", labels);

        f_plota("Test array", vx.x, vy0.x, vx.size, "x", labels[0]);
        f_plotas("Test arrays", x, ys, vx.size, 2, "x", labels);
        return 0;
}
