...
void tr_plotv(const char *title, const tr_vecd *x, const tr_vecd *ys,
        ...
        tr_plotvs(title, x, &ys, 1, xLabel, &label);
}
...
void tr_plotvs(const char *title, const tr_vecd *x, const tr_vecd **ys,
        ...
        char *path = tr_allocPush(&alloc, 2048);
        tr_getFilePath(path, title);
        ...
        for (size_t i = 0; i < nvecs; ++i) {
                double M = tr_vecdMax(ys[i]);
                double m = tr_vecdMin(ys[i]);
                if (upLim < M) {
                        upLim = M;
                }
                if (downLim > m) {
...
void tr_plota(const char *title, const double *xa, const double *ya,
        ...
        tr_plotv(title, &xv, &yv, xLabel, label);
}

void tr_plotas(const char *title, const double *xa, const double **yas,
               const size_t rows, const size_t nvecs, const char *xLabel,
               const char **labels) {
        tr_vecd xv = {.size = rows, .x = (double *)xa};

        tr_vecd **ys_ptrs = malloc(sizeof(tr_vecd *) * nvecs);
        tr_vecd *ys_vals = malloc(sizeof(tr_vecd) * nvecs);
        ...
        tr_plotvs(title, &xv, (const tr_vecd **)ys_ptrs, nvecs, xLabel, labels);
        free(ys_vals);
        free(ys_ptrs);
}
