...
void rk4Stepv(tr_vecd *ynext, dvdt_fn dvdt, const double x,
              tr_vecd *__restrict__ yn, const double h, const void *params,
              tr_rk4Containers *rk4containers) {
        tr_vecd *__restrict__ k1 = rk4containers->k1;
        tr_vecd *__restrict__ k2 = rk4containers->k2;
        tr_vecd *__restrict__ k3 = rk4containers->k3;
        tr_vecd *__restrict__ k4 = rk4containers->k4;
        tr_vecd *__restrict__ ytemp = rk4containers->ytemp;
...
void tr_rk4v(tr_alloc *alloc, dvdt_fn dvdt, tr_matd *__restrict__ Y,
             const tr_vecd *__restrict__ y0, const tr_vecd *__restrict__ x,
             const double h, const void *functionParams) {
        tr_cudaAssert(alloc != NULL);
        tr_cudaAssert(dvdt != NULL);
        tr_cudaAssert(Y != NULL);
        tr_cudaAssert(y0 != NULL);
        tr_cudaAssert(x != NULL);
        tr_cudaAssert(functionParams != NULL);

        tr_cudaAssert(x->size == Y->cols);
        tr_cudaAssert(y0->size == Y->rows);
